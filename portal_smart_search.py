...
const realName=v=>{const s=String(v??'').trim();return s&&!/^im[oó]vel rural\b/i.test(s)?s:''};
function clear(){q('#rxSmartResults')?.remove()}
function card(items,term,title='RESULTADOS'){clear();const box=document.createElement('div');box.id='rxSmartResults';box.className='rx-smart-results';box.innerHTML=`<div class="rx-smart-title">${esc(title)} PARA “${esc(term)}”</div>`;(items||[]).forEach(x=>{const b=document.createElement('button');b.className='rx-smart-item';const label=x.type==='car'?(realName(x.name)||String(x.car_code||'').trim().toUpperCase()):String(x.name??'').trim();const place=[String(x.municipality||x.state||'').trim(),String(x.uf||'').trim()].filter(Boolean).join(' / ');const meta=[place,window.rxNum?window.rxNum.ha(x.area_ha):'',x.registry?'registro '+String(x.registry):''].filter(Boolean).join(' · ');b.innerHTML=`<span class="rx-smart-badge">${x.type==='car'?'CAR':(x.type==='city'?'CIDADE':'SIGEF')}</span><b>${esc(label)}</b>${meta?`<small>${esc(meta)}</small>`:''}`;b.onclick=()=>x.type==='city'?openCity(x):openResult(x);box.appendChild(b)});if(!(items||[]).length){const d=document.createElement('div');d.className='rx-smart-item';d.innerHTML='<small>Nenhum resultado encontrado nas bases disponíveis.</small>';box.appendChild(d)}document.body.appendChild(box)}
