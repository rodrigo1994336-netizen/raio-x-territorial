...
function card(items,term){clear();const box=document.createElement('div');box.id='rxSmartResults';box.className='rx-smart-results';box.innerHTML=`<div class="rx-smart-title">RESULTADOS PARA “${String(term).replace(/[<>]/g,'')}”</div>`;(items||[]).forEach(x=>{const b=document.createElement('button');b.className='rx-smart-item';b.innerHTML=`<span class="rx-smart-badge">${x.type==='car'?'CAR':'SIGEF'}</span><b>${x.name||'Imóvel rural'}</b><small>${x.municipality||''}${x.uf?' / '+x.uf:''}${x.area_ha!=null?' · '+x.area_ha+' ha':''}${x.registry?' · registro '+x.registry:''}</small>`;b.onclick=()=>openResult(x);box.appendChild(b)});if(!(items||[]).length){const d=document.createElement('div');d.className='rx-smart-item';d.innerHTML='<small>Nenhuma fazenda/identificador encontrado nas bases nominais públicas. Tentando município…</small>';box.appendChild(d)}document.body.appendChild(box)}
...
async function cityFallback(term){try{const r=await fetch(`/v1/live/cities?q=${encodeURIComponent(term)}`),d=await r.json();if(r.ok&&d.items?.length&&typeof cityResults==='function'){cityResults(d.items);return true}}catch(e){}return false}
async function search(term){const raw=String(term||'').trim();if(!raw)return;clear();if(typeof carPattern==='function'&&carPattern(raw)){if(typeof loadCar==='function')return loadCar(raw.toUpperCase());}try{const r=await fetch(`/v1/live/search/properties?q=${encodeURIComponent(raw)}&limit=20`),d=await r.json();if(r.ok&&d.items?.length){card(d.items,raw);return}card([],raw);setTimeout(()=>cityFallback(raw),250)}catch(e){await cityFallback(raw)}}
...
print('RX_SMART_SEARCH=farm_name_car_city_identifiers',flush=True)
