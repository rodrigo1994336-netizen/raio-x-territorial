...
def _query_names_sync(west: float, south: float, east: float, north: float, limit: int = 60) -> dict[str, Any]:
    ...
    if not raw.get('ok'):
        return {'ok': False,'items': [],'count': 0,'source': 'SIGEF/INCRA — espelho público IBAMA/PAMGIA','detail': raw.get('detail') or raw.get('preview') or 'fonte_indisponivel'}
    data = raw.get('json') or {};features = data.get('features') or [];items=[];seen=set()
    for feature in features:
        ...
        parcel=str(props.get('parcela_co') or '').strip();registry=props.get('registro_m') or props.get('registro_d')
        dedupe=parcel or f"{name.upper()}|{round(center['lat'],5)}|{round(center['lon'],5)}"
        if dedupe in seen:continue
        seen.add(dedupe)
        items.append({'name':name,'municipality':props.get('municipio_'),'uf':props.get('uf_id'),'parcel_code':props.get('parcela_co'),'property_code':props.get('codigo_imo'),'registry':registry,'status':props.get('status') or props.get('situacao_i'),'center':center,'source':'SIGEF/INCRA — espelho público'})
        if len(items)>=cap:break
    items.sort(key=lambda x:(str(x.get('name') or '').upper(),str(x.get('municipality') or '').upper()))
    out={'ok':True,'items':items,'count':len(items),'candidate_count':len(features),'truncated':len(features)>len(items) and len(items)>=cap,'source':'SIGEF/INCRA — espelho público IBAMA/PAMGIA','cached':False,'note':'Denominações públicas de áreas certificadas. O sistema não inventa nomes nem presume titularidade.'}
    _CACHE[key]=(now,out)
    if len(_CACHE)>300:
...
async def property_names_viewport(west:float,south:float,east:float,north:float,limit:int=60):
...
print('RX_PROPERTY_NAMES_V40=broader_viewport_sigef_public_names_cached', flush=True)
