...
def _load_report_v10_after_report_api():
    for _ in range(320):
        ...
        if mod is not None and hasattr(mod,'app') and hasattr(mod,'_analyze_with_live_addons'):
            try:
                ...
                import report_v10_patch  # noqa: F401
                print('RX_REPORT_V10_RUNTIME=loaded_deferred',flush=True)
            except Exception as exc:
                print(f'RX_REPORT_V10_RUNTIME=failed:{type(exc).__name__}:{str(exc)[:300]}',flush=True)
            return
        time.sleep(0.05)
...
threading.Thread(target=_load_report_v10_after_report_api,daemon=True).start()
if os.getenv('RX_RELEASE') == 'V8_OPERATIONAL_ZERO_COST':
