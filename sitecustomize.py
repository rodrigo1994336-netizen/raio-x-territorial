            ...
            try:
                ...
                for required in ('/sw.js','/v1/live/property-names/viewport','/v1/live/territorial-production/{car_code}','/v1/live/critical-minerals/{car_code}'):
                    if required not in ready:
                        raise RuntimeError('missing_v36_route:' + required)
                html = portal_v8.PORTAL_HTML
                for marker in ('RX_PREMIUM_UX_V35','RX_PREMIUM_INTERACTIONS_V36'):
                ...
                guard.mark_ready()
                print(f'RX_PORTAL_V36_EXTENSION=loaded_deferred routes:{len(ready)} field_mode:on farm_names:on premium_ux:on interactions:on', flush=True)
            except Exception as exc:
                if guard is not None:
                    try: guard.mark_failed(exc)
                    except Exception: pass
                print(f'RX_PORTAL_V36_EXTENSION=failed:{type(exc).__name__}:{str(exc)[:500]}', flush=True)
            return
        time.sleep(0.05)
    print('RX_PORTAL_V36_EXTENSION=timeout_waiting_portal_api', flush=True)


if CORE_RUNTIME_READY:
    if IS_PORTAL:
        threading.Thread(target=_load_portal_v36_deferred, daemon=True).start()
    else:
