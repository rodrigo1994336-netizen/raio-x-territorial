...
def _panel_sync_v48(car_code: str, *, cancel_event=None) -> dict[str, Any]:
    ...
    mte.update({
        "state": "on_demand",
        "reason": "A lista do Ministério do Trabalho é de empregadores, não de imóveis; o CAR público não diz quem é o dono.",
    })
...
(function(){
 ...
 function buildAudit(card){let box=card.querySelector('.rx48-audit-box');if(!box){box=document.createElement('div');box.className='rx48-audit-box';card.querySelector('.rx45-audit-count')?.after(box)}const a=auditModel(card),R=readingOf(card),items=[];
  if(!a){items.push(auditItem('Fontes','CONSULTA PENDENTE','A lista de fontes desta consulta não pôde ser montada agora.'))}
  ...
  else if(R)items.push(auditItem('Análise completa','CONSULTA PENDENTE','As fontes oficiais não responderam agora.'));
