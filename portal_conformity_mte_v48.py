...
# The eight deep checks of the approved Conformidade Real front. Only MTE is
# implemented in this first slice. The other seven remain explicitly gray.
_FUTURE = [
    {"id": "sinaflor", "label": "SINAFLOR — autorização", "state": "not_consulted", "reason": "Ainda não consultada nesta etapa. Não foi inferida ausência de autorização."},
    {"id": "ibama_embargo", "label": "IBAMA — embargos", "state": "not_consulted", "reason": "Ainda não consultada por área nesta etapa."},
    {"id": "ibama_auto", "label": "IBAMA — autos", "state": "not_consulted", "reason": "Ainda não consultada por área nesta etapa."},
    {"id": "icmbio_embargo", "label": "ICMBio — embargos", "state": "not_consulted", "reason": "Ainda não consultada por área nesta etapa."},
    {"id": "icmbio_auto", "label": "ICMBio — autos", "state": "not_consulted", "reason": "Ainda não consultada por área nesta etapa."},
    {"id": "incra_settlement", "label": "INCRA — assentamento", "state": "not_consulted", "reason": "Ainda não consultada por interseção territorial nesta etapa."},
    {"id": "incra_quilombola", "label": "INCRA — quilombola", "state": "not_consulted", "reason": "Ainda não consultada por interseção territorial nesta etapa."},
]
...
.rx45-check{align-items:flex-start!important;min-height:54px!important;padding:8px!important}
...
@media(max-width:390px){.rx45-check{min-height:0!important}.rx48-check-label{font-size:8.5px}.rx48-check-reason{font-size:7.5px}}
</style>
<script>
(function(){
 ...
 const future={
  sinaflor:['SINAFLOR — autorização','Ainda não consultada nesta etapa. Não foi inferida ausência de autorização.','IBAMA / SINAFLOR'],
  ibama_embargo:['IBAMA — embargos','Ainda não consultada por área nesta etapa.','IBAMA'],
  ibama_auto:['IBAMA — autos','Ainda não consultada por área nesta etapa.','IBAMA'],
  icmbio_embargo:['ICMBio — embargos','Ainda não consultada por área nesta etapa.','ICMBio'],
  icmbio_auto:['ICMBio — autos','Ainda não consultada por área nesta etapa.','ICMBio'],
  incra_settlement:['INCRA — assentamento','Ainda não consultada por interseção territorial nesta etapa.','INCRA'],
  incra_quilombola:['INCRA — quilombola','Ainda não consultada por interseção territorial nesta etapa.','INCRA']
 };
 ...
 function updateCounter(card){if(!card)return;const audit=card.querySelector('.rx45-audit-count');if(!audit)return;if(!audit.dataset.rx48Base){const m=(audit.textContent||'').match(/(\d+)\s+de\s+10/i);audit.dataset.rx48Base=String(m?Number(m[1]):2)}const base=Number(audit.dataset.rx48Base||2);const answered=card.querySelectorAll('.rx45-check[data-answered="1"]').length;const button=audit.querySelector('#rx45Audit');audit.childNodes.forEach(n=>{if(n.nodeType===3)n.remove()});audit.insertBefore(document.createTextNode(`${base+answered} de 10 fontes responderam · `),button||null)}
 ...
 function enhance(card){if(!card||card.dataset.rx48Enhanced==='1')return;card.dataset.rx48Enhanced='1';card.querySelectorAll('.rx45-check').forEach(el=>{const id=el.dataset.source||'';el.dataset.answered='0';if(id==='mte_slave_labor'){el.dataset.state='on_demand';body(el,'MTE — Trabalho Escravo','NÃO VERIFICADA','Depende de CPF/CNPJ do proprietário; o CAR não fornece esse vínculo com segurança.','Fonte: Ministério do Trabalho e Emprego · dado: aguardando consulta')}else if(future[id]){const [label,reason,source]=future[id];el.dataset.state='not_consulted';body(el,label,'NÃO CONSULTADA',reason,`Fonte: ${source} · dado: não consultado · consulta: não realizada`)}});updateCounter(card);loadMte(card)}
 const obs=new MutationObserver(()=>document.querySelectorAll('.rx45-panel-card').forEach(enhance));function start(){obs.observe(document.body,{subtree:true,childList:true});document.querySelectorAll('.rx45-panel-card').forEach(enhance)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();
...
print("RX_PORTAL_CONFORMITY_MTE_V48=async_owner_truth_no_false_green", flush=True)
