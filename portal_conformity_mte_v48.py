...
(function(){
 ...
 async function loadMte(card){const el=card.querySelector('.rx45-check[data-source="mte_slave_labor"]');if(!el||el.dataset.rx48Loaded==='1')return;el.dataset.rx48Loaded='1';setState(el,'checking','MTE — Trabalho Escravo','CONSULTANDO','Validando a lista oficial do MTE. Nenhum resultado será presumido antes da resposta.','Fonte: Ministério do Trabalho e Emprego',false);const car=card.dataset.car||'';try{const r=await fetch(`/v1/live/conformity/mte/${encodeURIComponent(car)}`),d=await r.json();if(!r.ok)throw new Error(d.detail||`HTTP ${r.status}`);if(d.state==='blocked_missing_owner_identity'){setState(el,d.state,'MTE — Trabalho Escravo','NÃO VERIFICADA',d.reason,`Fonte: MTE${d.data_date?` · dado/publicação: ${fmtDate(d.data_date)}`:''} · consulta: ${fmtQuery(d.queried_at)}`,false)}else if(d.state==='checked_clear'){setState(el,d.state,'MTE — Trabalho Escravo','SEM OCORRÊNCIA',d.reason,`Fonte: MTE${d.data_date?` · dado/publicação: ${fmtDate(d.data_date)}`:''} · consulta: ${fmtQuery(d.queried_at)}`,true)}else if(d.state==='checked_hit'){setState(el,d.state,'MTE — Trabalho Escravo','OCORRÊNCIA LOCALIZADA',d.reason,`Fonte: MTE${d.data_date?` · dado/publicação: ${fmtDate(d.data_date)}`:''} · consulta: ${fmtQuery(d.queried_at)}`,true)}else{mtePending(card,el)}}catch(e){mtePending(card,el)}} function mtePending(card,el){if(el.dataset.rx48Retried!=='1'){el.dataset.rx48Retried='1';el.dataset.rx48Loaded='';setTimeout(()=>{if(el.isConnected)loadMte(card)},4000);return}setState(el,'source_failed','MTE — Trabalho Escravo','CONSULTA PENDENTE','A fonte oficial não respondeu agora. Nenhum resultado foi presumido.','Fonte: Ministério do Trabalho e Emprego',false)}
 ...
 if(!window.__rx48RetryWired){window.__rx48RetryWired=1;document.addEventListener('click',e=>{const b=e.target.closest?.('[data-rx48-retry="mte_slave_labor"]');if(!b)return;e.preventDefault();const el=b.closest('.rx45-check'),card=b.closest('.rx45-panel-card');if(!el||!card)return;el.dataset.rx48Loaded='';el.dataset.rx48Retried='1';loadMte(card)})}window.rxV48UpdateComplianceSource=function(id,result){const el=q(`.rx45-check[data-source="${id}"]`);if(!el)return;setState(el,result.state||'not_consulted',result.label||id,result.status||'NÃO CONSULTADA',result.reason||'',result.meta||'',!!result.answered)};
})();
</script>
'''
