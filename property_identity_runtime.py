...
def _osm_named_farms_bbox(west:float,south:float,east:float,north:float,limit:int=60)->dict[str,Any]:
    """Return explicit OSM place=farm names in a small bbox; fail soft on network errors."""
    ...
    for endpoint in _OSM_ENDPOINTS:
        try:
            req=Request(
                ...
                data=body,
                headers={
                    'User-Agent':'Raio-X-Territorial/V43.8 (+public-name-resolution)',
            ...
            for element in data.get('elements') or []:
                ...
                seen.add(key)
                items.append({
                    'name':name,'lat':float(lat),'lon':float(lon),'osm_type':'node','osm_id':node_id,
                    'place':tags.get('place'),'source':_OSM_SOURCE,
                })
...
def _osm_identity_candidate(car_geom:dict[str,Any],bbox:list[float])->dict[str,Any]:
    if not car_geom or not bbox or len(bbox)!=4:
        return {'ok':False,'chosen':None,'items':[],'detail':'missing_geometry_or_bbox'}
    try:
        west,south,east,north=[float(x) for x in bbox]
        car=shape(car_geom)
        if car.is_empty:return {'ok':False,'chosen':None,'items':[],'detail':'empty_car_geometry'}
    except Exception as exc:
        return {'ok':False,'chosen':None,'items':[],'detail':f'geometry:{type(exc).__name__}:{exc}'}
    osm=_osm_named_farms_bbox(west,south,east,north,60)
    if not osm.get('ok'):
        return {'ok':False,'chosen':None,'items':[],'detail':osm.get('detail'),'source':_OSM_SOURCE}
    inside=[]
    ...
    # A single unique explicit geographic name inside the exact CAR is required.
    # Multiple distinct names in the same CAR are treated as ambiguous and never promoted.
    if len(by_name)==1:
        group=next(iter(by_name.values()));first=group[0]
        chosen={**first,'evidence_count':len(group)}
        return {'ok':True,'chosen':chosen,'items':inside,'conflict':False,'source':_OSM_SOURCE}
    return {
        'ok':True,'chosen':None,'items':inside,'conflict':len(by_name)>1,
        'names':sorted({x['name'] for x in inside},key=str.casefold),'source':_OSM_SOURCE,
    }
...
def resolve_property_identity_sync(car_code:str)->dict[str,Any]:
    ...
    if direct:
        out={'ok':True,'car_code':code,'name':direct,'source':'SICAR','confidence':'high','method':'explicit_sicar_field','candidates':[],'candidate_count':0}
        _CACHE[code]=(now,out);return out
    ...
    if chosen:
        ...
        osm_chosen=osm.get('chosen')
        if osm_chosen:
            out={
                'ok':True,'car_code':code,'name':osm_chosen['name'],'source':_OSM_SOURCE,
                'confidence':'medium','method':'osm_named_farm_point_inside_car',
                'osm_node_id':osm_chosen.get('osm_id'),'osm_lat':osm_chosen.get('lat'),'osm_lon':osm_chosen.get('lon'),
                'evidence_count':osm_chosen.get('evidence_count',1),'candidates':items[:5],
                'candidate_count':len(items),'osm_candidates_inside_car':len(osm.get('items') or []),
                'note':'Denominação geográfica pública associada por ponto nomeado contido no polígono exato do CAR; não implica titularidade.',
            }
        else:
            out={
                ...
                'candidate_count':len(items),'osm_candidates_inside_car':len(osm.get('items') or []),
                'osm_conflict':bool(osm.get('conflict')),
...
print('RX_PROPERTY_IDENTITY=public_name_resolver_sicar_sigef_osm_point_inside_car',flush=True)
