...
def build_live_payload(result: dict[str, Any], report_id: str, generated_at: str, map_path: str) -> dict[str, Any]:
    ...
    overall = 'ALTO' if emb_count else ('MODERADO' if prodes_count else 'BAIXO')
    ...
    payload = {
        ...
        'quick_read': f'CAR real localizado em {_s(props.get("municipio"))}/{_s(props.get("uf"))}, com {round(area_ha,3)} ha. A consulta encontrou {prodes_count} ocorrência(s) PRODES em interseção exata, {(str(emb_count) + " embargo(s) IBAMA") if emb_ok else "embargo IBAMA com consulta pendente"} e {anm_count} processo(s) ANM. Fontes ainda não ativadas aparecem explicitamente como NÃO CONSULTADO.',
        'attention_points': [
