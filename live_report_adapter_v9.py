...
def _replace_source(payload,name_contains,new_row):
    terms=[x.lower() for x in name_contains]
    kept=[]
    for src in payload.get('sources') or []:
        n=str(src.get('name') or '').lower()
        if any(t in n for t in terms): continue
        kept.append(src)
    kept.append(new_row)
...
def _patch_ide(payload:dict,result:dict):
    ...
    # SOLO
    soil=ide.get('soil') or {}
    st,level=_layer_status(soil)
    props=_sample_props(soil)
    ...
    # APTIDÃO
    apt=ide.get('aptitude') or {}
    st,level=_layer_status(apt); props=_sample_props(apt)
    arows=[]
    for p in props[:8]:
    ...
    # DECLIVIDADE
    slope=ide.get('slope') or {}
    st,level=_layer_status(slope); props=_sample_props(slope)
    slope_values=[]
    for p in props[:6]:
        for key in ('classe','decliv','intervalo','legenda','gridcode'):
            if p.get(key) not in (None,''):
                slope_values.append(f"{key}: {p.get(key)}")
    if slope.get('ok'):
        sval='; '.join(_uniq(slope_values)) or f"{slope.get('exact_count',0)} feição(ões) intersectante(s)"
        snote='IDE-Sisema'
    elif 'payload_guard' in str(slope.get('detail') or ''):
        sval='PARCIAL'
        snote='A camada respondeu acima do limite seguro de payload; o resultado não foi interpretado como ausência de declividade.'
    else:
        sval=st; snote=slope.get('detail') or 'Fonte indisponível.'
    terrain=prod.get('terrain_kpis') or []
    # Remove placeholder de declividade e preserve outros indicadores reais se existirem.
    ...
    # EROSÃO
    ero=ide.get('erosion') or {}
    st,level=_layer_status(ero); props=_sample_props(ero)
    ...
    # USO E COBERTURA
    cover_key=next((k for k in ide if str(k).startswith('landcover')),None)
    cover=ide.get(cover_key) or {}
    st,level=_layer_status(cover); props=_sample_props(cover)
    ...
    # RECOMPOSIÇÃO DE RESERVA LEGAL DECLARADA
    rl=ide.get('rl_recomposition') or {}
    st,level=_layer_status(rl); props=_sample_props(rl)
    rl_rows=[]
    for p in props[:8]:
    ...
    if rl.get('ok'):
        # Important: features returned in bbox may belong to neighboring CARs. Only call
        # them evidence of the spatial query; do not automatically attribute to selected CAR.
        env.setdefault('layer_rows',[]).extend(rl_rows[:4])
    ...
    # Remove mensagens antigas de que estas camadas ainda não entraram, pois isso
    # contradiz a execução real.
    cleaned=[]
    ...
    for row in payload.get('compliance') or []:
        if str(row.get('label') or '').lower()=='solo / aptidão':
            ...
            row['badge']='CONSULTADO' if soil.get('ok') and apt.get('ok') else 'PARCIAL'
            row['level']='ok' if row['badge']=='CONSULTADO' else 'attention'
    for row in (payload.get('conclusion') or {}).get('categories') or []:
        if row.get('label')=='Produtivo':
            ...
            row['risk']='ATENÇÃO' if not (soil.get('ok') and apt.get('ok')) else 'TRIAGEM DISPONÍVEL'
            row['level']='attention' if row['risk']=='ATENÇÃO' else 'info'
    return payload


def _dedupe_sources(payload:dict):
    # Keep the most recent row for repeated modules so the final status represents
    # the actual executed pipeline, not a placeholder created earlier.
    ordered=[]; pos={}
    for src in payload.get('sources') or []:
        ...
        if key in pos:
            ...
            pos[key]=len(ordered); ordered.append(src)
    payload['sources']=ordered
    return payload


def _final_truth_guard(payload:dict,result:dict):
    # If a module exists in the runtime result, report its actual state. This guard
    # prevents stale 'NÃO CONSULTADO' placeholders from surviving after a real query.
    replacements={
        'outorgas': result.get('water_mg'),
        'pivôs': result.get('pivots_ana'),
        'clima': result.get('climate_nasa'),
    }
    for src in payload.get('sources') or []:
        n=str(src.get('name') or '').lower()
        for term,obj in replacements.items():
            if term in n and isinstance(obj,dict):
                if obj.get('ok') is True and str(src.get('status') or '').upper() in {'NÃO CONSULTADA','NÃO CONSULTADO','NAO CONSULTADA','NAO CONSULTADO'}:
...
def generate_live_report(result:dict,car_code:str):
    ...
    payload=_patch_ide(payload,result)
    payload=_dedupe_sources(payload)
    payload=_final_truth_guard(payload,result)
    payload['source_version']='Raio-X Territorial V9 • matriz de cobertura por fonte + relatório de layout fluido sem posicionamento absoluto de texto.'
