...
from live_report_adapter_v9 import _patch_ide, _dedupe_sources, _final_truth_guard
...
def _patch_extra_territorial(payload:dict,result:dict):
    cons=result.get('territorial_constraints') or {}
    services=cons.get('services') or {}
    env=payload.setdefault('environment',{})
    total=float((result.get('car') or {}).get('properties',{}).get('area') or 0)
    labels={
        'floresta_publica':'Floresta Pública — SFB',
        'sitio_arqueologico':'Sítio Arqueológico — IPHAN',
    }
    for key,label in labels.items():
        r=services.get(key) or {}
        source=r.get('source') or ('SFB' if key=='floresta_publica' else 'IPHAN')
        if r.get('ok'):
...
def _patch_restricted_parity(payload:dict):
    # Restricted/paid capabilities are visible and auditable in the report instead of
    # disappearing. They remain OFF until legal access/credentials are activated.
    pstat=premium_status()
    by_code={x['code']:x for x in pstat.get('integrations') or []}
    mapping=[
    ...
    for code,label,desc in mapping:
        ...
        payload.setdefault('sources',[]).append({
            'name':label,
            'description':desc + (' Integração ativa nesta emissão.' if ready else ' Integração já preparada na arquitetura; ativação pendente de credencial/habilitação.'),
            'status':'CONSULTADA' if ready else 'INTEGRAÇÃO PREPARADA — OFF',
            'level':'ok' if ready else 'neutral',
        })
    return payload
...
def generate_live_report(result:dict,car_code:str):
    ...
    payload=_patch_extra_territorial(payload,result)
    ...
    payload=_patch_restricted_parity(payload)
    ...
    payload['source_version']='Raio-X Territorial V11 • dados reais + cobertura por fonte + SFB + IPHAN + integrações restritas explicitamente preparadas • layout fluido e acessível.'
...
print('RX_LIVE_REPORT_ADAPTER=V11_PARITY_PUBLIC',flush=True)
