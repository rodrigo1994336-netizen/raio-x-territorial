...
async def build_agro_profile(result:dict,car_code:str,include_sif:bool=True) -> dict[str,Any]:
    ...
    if include_sif:
        ...
        ppm=await query_ppm(car_code);sif={'ok':False,'state':'not_requested_in_lightweight_mode','source':'MAPA / SIGSIF'}
    return {
        'ok':bool(ppm.get('ok') or sif.get('ok')),'source':'IBGE/PPM + MAPA/SIGSIF + fontes territoriais já consultadas pelo Raio-X','municipality':municipality,'uf':uf,
        'livestock_municipal':ppm,'sif_chain':sif,'property_screening':property_livestock_screening(result),
