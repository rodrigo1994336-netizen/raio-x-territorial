...
never got an answer (timeout, connection error, service exception, a filter the service
ignored) is a pending consultation: 503 with Retry-After, so the portal and the proxies
retry instead of telling the client that the property does not exist (tentar não é
responder). A code that cannot be a CAR is 422: SICAR was never asked.
...
INVALID_DETAIL = "Código CAR inválido."
RETRY_AFTER_SECONDS = 30


def lookup_http_error(car: dict | None, *, not_found_detail=NOT_FOUND_DETAIL, pending_detail=PENDING_DETAIL,
                      invalid_detail=INVALID_DETAIL) -> HTTPException:
    """422 for an impossible code, 404 only when SICAR answered without the property, 503 + Retry-After otherwise."""
    car = car if isinstance(car, dict) else {}
    if car.get('detail') == 'invalid_car_format':
