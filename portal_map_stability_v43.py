...
parcel_pattern = re.compile(
    r"if\(rxParcelLayer\)m\.removeLayer\(rxParcelLayer\);rxParcelLayer=L\.geoJSON\(d,\{.*?\}\)\.addTo\(m\);setMapState\(",
    re.S,
)
if len(parcel_pattern.findall(html)) != 1:
...
html = parcel_pattern.sub(lambda _: parcel_reconcile, html, count=1)
