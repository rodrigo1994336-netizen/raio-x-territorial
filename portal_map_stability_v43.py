...
parcel_reconcile = r"""if(!rxParcelLayer)rxParcelLayer=L.layerGroup().addTo(m);const rxTipC2=p=>{const e=s=>String(s??'').replace(/[&<>"']/g,c=>'&#'+c.charCodeAt(0)+';'),city=String(p?.municipality||'').trim(),uf=String(p?.uf||'').trim(),area=window.rxNum?window.rxNum.ha(p?.area_ha):'';return [city?(uf?city+' / '+uf:city):'',area].filter(Boolean).map(e).join(' · ')};const rxNextParcels=new Set();const rxParcelStyleFor=f=>(window.rxParcelStyle?window.rxParcelStyle(f):{color:'#48d995',weight:1.4,fillColor:'#48d995',fillOpacity:.075});const rxParcelKey=(f,i)=>{const p=propertyFromFeature(f);return String(p.car_code||f?.id||('anon:'+i+':'+(p.municipality||'')+':'+(p.area_ha??'')))};const rxGeomSig=f=>{try{return JSON.stringify(f?.geometry?.coordinates||[]).length}catch(e){return 0}};const rxBuildParcel=(f,key)=>{const group=L.geoJSON(f,{style:rxParcelStyleFor,onEachFeature:(ff,l)=>{l.bindTooltip('',{sticky:true});l.on('click',e=>{if(e.originalEvent)L.DomEvent.stopPropagation(e.originalEvent);const live=l.feature||ff,p=propertyFromFeature(live);if(typeof showProperty==='function')showProperty(p,live.geometry)})}});group.__rxGeomSig=rxGeomSig(f);group.eachLayer(l=>{const p=propertyFromFeature(f);l.feature=f;l.setTooltipContent?.(rxTipC2(p))});group.addTo(rxParcelLayer);rxParcelIndex.set(key,group);return group};(d.features||[]).forEach((f,i)=>{const key=rxParcelKey(f,i);rxNextParcels.add(key);let group=rxParcelIndex.get(key),sig=rxGeomSig(f);if(!group){rxBuildParcel(f,key);return}if(group.__rxGeomSig!==sig){const fresh=rxBuildParcel(f,key);try{rxParcelLayer.removeLayer(group)}catch(e){}rxParcelIndex.set(key,fresh);return}group.eachLayer(l=>{l.feature=f;try{l.setStyle?.(rxParcelStyleFor(f))}catch(e){}const p=propertyFromFeature(f);try{l.setTooltipContent?.(rxTipC2(p))}catch(e){}})});for(const [key,group] of [...rxParcelIndex.entries()]){if(rxNextParcels.has(key))continue;try{rxParcelLayer.removeLayer(group)}catch(e){}rxParcelIndex.delete(key)}"""
