...
async def _report_name(car_code:str,property_name:str|None,request=None)->str:
    supplied=_provided_name(property_name)
    if supplied:return supplied
    try:
        # Dentro do escopo, na mesma forma da rota /v1/live/property-identity, que resolve o MESMO nome:
        # o cancel_event chega a cada curl da cadeia (SICAR + INCRA + OSM) e a desistencia do cliente
        # derruba todos. Sem prazo externo de propria lavra: cada trecho ja aplica o seu, e um teto escrito
        # aqui cortaria nome que hoje chega. Sem nome o relatorio se identifica pelo codigo do CAR — a
        # regra do dono —, entao a desistencia nunca vira nome errado.
        identity=await run_sync_with_request_lifecycle(
            request,resolve_property_identity_sync,str(car_code or '').upper(),timeout_seconds=None)
...
async function status(){{const r=await fetch(`/v1/mobile/report/status/${{encodeURIComponent(code)}}?property_name=${{encodeURIComponent(name)}}`,{{cache:'no-store'}}),d=await r.json().catch(()=>({{}}));if(!r.ok)throw new Error('status');return d}}
// V2: the job keeps the lookup answer (status_code). 404 = SICAR answered without the property; 503 = SICAR did not
// answer (consulta pendente): one quiet automatic retry after Retry-After. Never the technical text of a failure.
function view(title,text,busy,retry){{qs('#rxViewerStatus').textContent=title;qs('#rxViewerTitle').textContent=title;detail(text);qs('.stage .box').classList.toggle('rx-idle',!busy);qs('#rxViewerRetry').hidden=!retry}}
let autoRetried=false;
async function start(auto){{
 if(auto!==true)autoRetried=false;
 view('Preparando relatório · '+code,'Acordando o motor e preparando o relatório. Você não precisa voltar ao aplicativo.',true,false);qs('#rxViewerTitle').textContent='Gerando relatório territorial';qs('#rxViewerStage').hidden=false;qs('#rxPdfFrame').hidden=true;wake();
 let fail={{code:0,after:0}};
 try{{
  const r=await fetch(`/v1/mobile/report/prepare/${{encodeURIComponent(code)}}?property_name=${{encodeURIComponent(name)}}`,{{method:'POST',cache:'no-store'}});await r.json().catch(()=>({{}}));
  if(r.ok){{for(let i=0;i<150;i++){{await sleep(i<8?1000:1800);const s=await status();if(s.state==='ready'){{qs('#rxViewerStatus').textContent='PDF pronto · '+code;qs('#rxOpenPdf').hidden=false;qs('#rxViewerStage').hidden=true;const f=qs('#rxPdfFrame');f.src=pdf;f.hidden=false;return}}if(s.state==='failed'){{fail={{code:Number(s.status_code)||0,after:Number(s.retry_after)||0}};break}}detail('Gerando relatório · '+Math.max(1,Math.round((i+1)*(i<8?1:1.8)))+'s')}}}}
 }}catch(e){{fail={{code:0,after:0}}}}
 if(fail.code===404){{view('CAR não localizado no SICAR.','Confira o código do CAR.',false,false);return}}
 if(fail.code===503&&!autoRetried){{autoRetried=true;const wait=Math.min(60,Math.max(3,Math.round(fail.after)||10));view('Consulta ao SICAR pendente','O SICAR não respondeu agora. Nova tentativa automática em '+wait+' s.',true,false);await sleep(wait*1000);return start(true)}}
 if(fail.code===503){{view('Consulta ao SICAR pendente','O SICAR não respondeu agora. Tente de novo em instantes.',false,true);return}}
 view('Relatório pendente','O relatório não pôde ser concluído agora. Tente de novo em instantes.',false,true)
}}
qs('#rxViewerRetry').onclick=()=>start(false);start(false);
