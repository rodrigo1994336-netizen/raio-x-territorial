...
(function(){
  ...
  async function openNamedProperty(x){const m=getMap();if(!m||!x?.center)return;const lat=Number(x.center.lat),lon=Number(x.center.lon);try{const r=await bounded(`/v1/live/resolve?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`,field()?6500:9000),d=await r.json();if(r.ok&&d?.property?.car_code&&typeof showProperty==='function'){d.property.name=x.name;showProperty(d.property,d.geometry);return}}catch(e){}if(typeof toast==='function')toast(`${x.name}: área pública localizada; nenhum CAR correspondente foi confirmado neste ponto.`)}
  function render(items,z){const m=getMap();if(!m)return 0;if(nameLayer){try{m.removeLayer(nameLayer)}catch(e){}}nameLayer=L.layerGroup();const limit=limitFor(z);let rendered=0;(items||[]).slice(0,limit).forEach(x=>{const lat=Number(x?.center?.lat),lon=Number(x?.center?.lon);if(!Number.isFinite(lat)||!Number.isFinite(lon)||!x?.name)return;const label=esc(x.name),low=z<=11?' rx-name-lowzoom':'';const icon=L.divIcon({className:'rx-farm-name-icon',html:`<div class="rx-farm-name-label${low}" title="${label}">${label}</div>`,iconSize:[1,1],iconAnchor:[0,0]});const mk=L.marker([lat,lon],{icon,keyboard:true,riseOnHover:true});mk.on('click',()=>openNamedProperty(x));mk.bindTooltip(`${esc(x.name)}${x.municipality?' · '+esc(x.municipality):''}${x.uf?' / '+esc(x.uf):''}${x.registry?' · Matrícula '+esc(x.registry):''}`,{direction:'top',offset:[0,-8]});nameLayer.addLayer(mk);rendered++});nameLayer.addTo(m);return rendered}
  async function refresh(){const m=getMap();if(!m||dossierOpen())return;const z=m.getZoom();if(z<11){clearNames();return}const b=m.getBounds(),west=b.getWest(),south=b.getSouth(),east=b.getEast(),north=b.getNorth();if(!(Number.isFinite(west)&&Number.isFinite(south)&&Number.isFinite(east)&&Number.isFinite(north)&&east>west&&north>south)){lastKey='';return}const span=Math.max(east-west,north-south);if(span>1.50){clearNames();return}const key=[west,south,east,north].map(v=>v.toFixed(3)).join('|')+'|'+z+'|'+(field()?'field':'normal');if(key===lastKey&&nameLayer)return;lastKey=key;const my=++seq;const u=new URL('/v1/live/property-names/viewport',location.origin);u.searchParams.set('west',west);u.searchParams.set('south',south);u.searchParams.set('east',east);u.searchParams.set('north',north);u.searchParams.set('limit',String(limitFor(z)));u.searchParams.set('diagnostic','true');const visibleCar=Number(window.rxVisibleCarCountV43);if(Number.isFinite(visibleCar)&&visibleCar>=0)u.searchParams.set('car_visible',String(Math.round(visibleCar)));try{const r=await bounded(u,field()?6500:9500),d=await r.json();if(my!==seq)return;if(!r.ok||!d.ok){lastKey='';sourceBadge(nameLayer?'Nomes já carregados · atualização pendente':'Nomes públicos indisponíveis agora');return}const rendered=render(d.items||[],z);window.rxNameCoverageV43={...(d.coverage||{}),car_visible:Number.isFinite(visibleCar)?visibleCar:(d.coverage?.car_visible??null),names_rendered:rendered,zoom:z,field_mode:field(),captured_at:new Date().toISOString()};sourceBadge(d.count?`${d.count} nome(s) público(s) · SIGEF/INCRA`:'Sem denominações públicas SIGEF nesta área')}catch(e){if(my===seq){lastKey='';sourceBadge(nameLayer?'Nomes já carregados · atualização pendente':'Nomes públicos indisponíveis agora')}}}
...
print('RX_PORTAL_PROPERTY_NAMES_V43=field_aware_controlled_density_coverage_funnel_viewport_guard', flush=True)
