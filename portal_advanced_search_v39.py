...
function render(items,profileMap={}){const box=host.querySelector('#rxAdvResults');box.innerHTML='';if(!items.length){box.innerHTML='<div class="rx-adv-card"><b>Nenhum imóvel encontrado</b><small>Altere cidade, área ou perfil produtivo.</small></div>';return}items.forEach(x=>{const p=profileMap[x.car_code]?.profile||null;const badges=p?.profiles?.slice(0,3)||[];const b=document.createElement('button');b.type='button';b.className='rx-adv-card';const rxTitle=String(x.name||x.car_code||'').trim(),rxPlace=[x.municipality,x.uf].filter(Boolean).join(' / '),rxArea=window.rxNum?window.rxNum.ha(x.area_ha):'',rxMeta=[rxPlace,rxArea].filter(Boolean).join(' · ');b.innerHTML=`<b>${esc(rxTitle)}</b>${rxMeta?`<small>${esc(rxMeta)}</small>`:''}<small>CAR ${esc(x.car_code||'—')}</small>${badges.length?`<div class="rx-adv-badges">${badges.map(y=>`<span class="rx-adv-badge">${esc(y.label)} · ${esc(y.share_pct)}%</span>`).join('')}</div>`:''}`;b.onclick=()=>mapOpen(x);box.appendChild(b)})}
async function search(){const uf=host.querySelector('#rxAdvUf').value,city=host.querySelector('#rxAdvCity').value.trim(),q=host.querySelector('#rxAdvQ').value.trim(),min=num('#rxAdvMin'),max=num('#rxAdvMax');if(!uf||city.length<2){status('Selecione a UF e informe o município.');return}if((min!=null&&!Number.isFinite(min))||(max!=null&&!Number.isFinite(max))){status('Revise os valores de área.');return}status('Localizando imóveis reais no município…',true);host.querySelector('#rxAdvResults').innerHTML='';const u=new URL('/v1/live/search/advanced',location.origin);u.searchParams.set('uf',uf);u.searchParams.set('municipality',city);u.searchParams.set('limit',selected?'16':'40');if(q)u.searchParams.set('q',q);if(min!=null)u.searchParams.set('min_area_ha',min);if(max!=null)u.searchParams.set('max_area_ha',max);try{const r=await fetch(u,{cache:'no-store'}),d=await r.json();if(!r.ok)throw new Error('sem resposta utilizável');lastItems=d.items||[];if(!selected){status(`${lastItems.length} imóvel(is) localizado(s) em ${d.municipality}/${d.uf}${d.truncated?' · refine os filtros para reduzir a lista':''}.`);render(lastItems);return}if(!lastItems.length){status('Nenhum imóvel candidato encontrado com esses filtros.');render([]);return}status(`Triagem produtiva em ${lastItems.length} imóvel(is)… os dados do mapa continuam disponíveis.`,true);const rr=await fetch('/v1/live/search/landuse-profiles',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({candidates:lastItems.map(x=>({car_code:x.car_code,geometry:x.geometry}))})}),pd=await rr.json();if(!rr.ok)throw new Error('sem resposta utilizável');const by={};(pd.items||[]).forEach(x=>by[x.car_code]=x);const matched=lastItems.filter(x=>(by[x.car_code]?.profile?.profile_codes||[]).includes(selected));status(`${matched.length} imóvel(is) compatível(is) com o perfil selecionado em ${d.municipality}/${d.uf}. Classificação por uso/cobertura do solo; não é declaração de atividade econômica.`);render(matched,by)}catch(e){status('A busca não respondeu agora. Tente de novo em instantes.')}}
