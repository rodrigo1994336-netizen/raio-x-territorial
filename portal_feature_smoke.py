...
async def _run():
    try:
        ...
        print('RX_RARE_EARTH_FILTER_SMOKE='+json.dumps({'ok':minerals.get('ok'),'layer_count':len(minerals.get('layers') or []),'source':minerals.get('source')},ensure_ascii=False,default=str),flush=True)
