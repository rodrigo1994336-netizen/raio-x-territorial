...
def _panel_sync(car_code: str) -> dict[str, Any]:
    ...
    sources = [
        ...
        },
        {
            "id": "denomination",
            "label": "Denominação",
            "state": "validated" if name_eligible else "unresolved",
            "detail": (
        ...
        {
            "id": "environment",
            "label": "Composição ambiental",
            "state": "on_demand",
            "detail": "Consulta aprofundada separada do painel rápido.",
        },
    ]
...
:root{--rx45-panel:410px;--rx45-bg:rgba(7,21,15,.965);--rx45-card:#0b2118;--rx45-card2:#102a1e;--rx45-line:#2a493b;--rx45-text:#eef8f2;--rx45-muted:#9fb5aa;--rx45-green:#63e6a5;--rx45-yellow:#f5c96a;--rx45-red:#ff927a}
...
.rx45-source{display:grid;grid-template-columns:10px minmax(90px,.65fr) minmax(0,1.35fr);gap:7px;align-items:start;padding:6px 0;border-top:1px solid #1d382c;font-size:8px;line-height:1.4}.rx45-source:first-of-type{border-top:0}.rx45-dot{width:7px;height:7px;margin-top:2px;border-radius:50%;background:var(--rx45-yellow)}.rx45-dot.validated,.rx45-dot.available{background:var(--rx45-green)}.rx45-dot.unresolved{background:var(--rx45-red)}.rx45-source em{font-style:normal;color:var(--rx45-muted)}
.rx45-reference{padding:7px 8px;border-radius:10px;background:#0c1d16;color:var(--rx45-muted);font-size:8px;line-height:1.45}.rx45-reference strong{color:#d8e9df}.rx45-actions{display:grid;grid-template-columns:1.25fr repeat(3,minmax(0,.75fr));gap:6px}.rx45-actions button{min-height:39px;border-radius:11px;border:1px solid var(--rx45-line);background:var(--rx45-card2);color:var(--rx45-text);font-size:8px;font-weight:900;cursor:pointer;padding:7px}.rx45-actions .primary{background:var(--rx45-green);border-color:var(--rx45-green);color:#052116}.rx45-actions button:disabled{opacity:.4;cursor:not-allowed}
...
@media(max-width:720px){
 ...
 .rx45-panel-card{border-radius:18px;padding:12px}.rx45-actions{grid-template-columns:repeat(2,minmax(0,1fr))}.rx45-actions .primary{grid-column:1/-1}.rx45-row{grid-template-columns:1fr;gap:2px}
}
</style>
<script>
(function(){
 ...
 function sourceRows(items){return (items||[]).map(x=>`<div class="rx45-source"><i class="rx45-dot ${esc(x.state||'on_demand')}"></i><b>${esc(x.label||'Fonte')}</b><em>${esc(x.detail||'')}</em></div>`).join('')}
 ...
 function bind(){q('#rx45Close')?.addEventListener('click',()=>window.rx43CloseDossier?.());q('#rx45Full')?.addEventListener('click',()=>{try{if(typeof window.rxProgressiveAnalyze==='function')window.rxProgressiveAnalyze();else if(typeof analyze==='function')analyze()}catch(e){}});q('#rx45Kml')?.addEventListener('click',downloadKml);q('#rx45Png')?.addEventListener('click',downloadPng);q('#rx45Pdf')?.addEventListener('click',()=>{try{window.downloadPDF?.()}catch(e){}})}
