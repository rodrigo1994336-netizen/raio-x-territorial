...
(function(){
 ...
 window.rxParcelStyle=function(){return {color:'#80c9aa',weight:1.25,opacity:.96,fillColor:'#55b889',fillOpacity:.16}};
 ...
 function action(e){const btn=e.target.closest?.('[data-rx46-action]');if(!btn)return;const act=btn.dataset.rx46Action;e.preventDefault();e.stopPropagation();if(act==='close')closeAnchor();else if(act==='full')openFull();else if(act==='kml'&&selected?.car_code)window.open(`/v1/exports/property/${encodeURIComponent(selected.car_code)}/kml`,'_blank');else if(act==='demo'&&selected?.car_code)window.open(`/v1/mobile/report/view/${encodeURIComponent(selected.car_code)}`,'_blank')}
 function install(){const m=mapRef();if(!m||typeof window.showProperty!=='function'){if((window.__rx46InstallAttempts=(window.__rx46InstallAttempts||0)+1)<=4)setTimeout(install,120);return}legacyOpen=window.showProperty;window.__rx46LegacyOpen=legacyOpen;window.showProperty=showProperty=function(p,g){window.rxV46SelectProperty(p,g,null)};document.addEventListener('click',action,true);try{const layers=window.rxBasemapLayersV43;if(layers?.roads)layers.roads.setZIndex(625);if(layers?.places)layers.places.setZIndex(630)}catch(e){}window.rxV46Installed=true}
