...
(function(){
 ...
 let rx46AnchorSeq=-1;
 function renderAnchor(latlng){const m=mapRef();if(!m||!selected||!latlng)return;if(!popup)popup=L.popup({className:'rx46-anchor-popup',closeButton:false,autoPan:false,closeOnClick:false,maxWidth:230,minWidth:210,offset:[0,-8]});const open=m.hasLayer(popup),el=open?popup.getElement():null,same=!!el&&rx46AnchorSeq===seq,h0=same?el.offsetHeight:0;const fa=same&&el.contains(document.activeElement)?document.activeElement:null,fsel=!fa?'':fa.matches('[data-rx-copy-fallback]')?'[data-rx-copy-fallback]':fa.matches('[data-rx-copy-car]')?'[data-rx-copy-car]':fa.dataset?.rx46Action?`[data-rx46-action="${fa.dataset.rx46Action}"]`:'';if(!same)popup.options.offset=[0,-8];popup.setLatLng(latlng).setContent(cardHtml(selected));if(!open)popup.openOn(m);rx46AnchorSeq=seq;if(same){const d=el.offsetHeight-h0;if(d){const o=popup.options.offset||[0,-8];popup.options.offset=[0,(Array.isArray(o)?o[1]:o.y)+d];popup.update()}if(fsel){try{el.querySelector(fsel)?.focus({preventScroll:true})}catch(e){}}}}
 async function enrich(localSeq,latlng){const car=String(selected?.car_code||'').trim().toUpperCase();if(!car)return;try{const r=await fetch(`/v1/live/map-panel/${encodeURIComponent(car)}`),d=await r.json();if(localSeq!==seq||!r.ok||!d?.ok)return;selected={...selected,...d,status:d.car_status||selected.status,type:d.property_type||selected.type,geometry:d.geometry||selected.geometry,__rx46Enriched:true};window.current={...selected};replaceSelectedLayer(selected.geometry);renderAnchor(latlng);sigefFollowUp(localSeq,latlng,car)}catch(e){}}
 // C2b: a reference query that did not answer gets ONE automatic retry while this same card is open.
 function sigefFollowUp(localSeq,latlng,car){const R=window.rxSigefRefC2;if(!R||!R.needsRetry(selected))return;R.scheduleRetry(car,()=>localSeq===seq&&!!popup&&!!mapRef()?.hasLayer(popup),d=>{if(localSeq!==seq||!selected)return;if(d)selected={...selected,...d,status:d.car_status||selected.status,type:d.property_type||selected.type,geometry:d.geometry||selected.geometry,__rx46Enriched:true};window.current={...selected};renderAnchor(latlng)})}
