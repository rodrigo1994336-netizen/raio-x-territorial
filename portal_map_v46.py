...
# Cache schema bump for W1a (bytes + cell mode); F1B7: features carry dat_criacao/data_atualizacao.
# Old entries must never collide with new ones.
_V46_VIEWPORT_CACHE_SCHEMA = "F1B7"
...
/* F1B 1B.7: a placeholder keeps the height of the field it waits for (status seal; condition in two lines, like 'Aguardando análise') */.rx46-wait{min-width:0}.rx46-wait small,.rx46-wait b{display:block;color:transparent;border-radius:4px;background:linear-gradient(90deg,#16302a 25%,#21443a 50%,#16302a 75%);background-size:200% 100%;animation:rx46Wait 1.3s ease-in-out infinite;user-select:none}.rx46-wait small{font-size:9px;margin-bottom:2px;width:56%}.rx46-wait b{font-size:11px;line-height:1.28;width:84%}.rx46-wait[data-rx46-wait="Status"] b{height:18px;width:52%}.rx46-wait[data-rx46-wait="Condição"] b{height:28px}.rx46-grid>.rx46-wait:last-child:nth-child(odd){grid-column:1/-1}@keyframes rx46Wait{from{background-position:100% 0}to{background-position:-100% 0}}@media (prefers-reduced-motion:reduce){.rx46-wait small,.rx46-wait b{animation:none}}
/* F1B: phone lying down (screen lower than the card): the card scrolls inside itself and the floating map controls step aside while it is open */@media (max-height:500px){.rx46-anchor-popup .rx46-card{max-height:calc(100vh - 110px);overflow-y:auto;overscroll-behavior:contain}body:has(.rx46-anchor-popup) #rxLocateBtn,body:has(.rx46-anchor-popup) #rxFilterBtn{visibility:hidden!important;pointer-events:none!important}}
</style>
...
(function(){
 ...
 function rx46CellAge(s,g){const t=(g&&typeof window.rxW1aCellFetchedAt==='function')?window.rxW1aCellFetchedAt(g):null;if(!Number.isFinite(t))return s;if(Date.now()-t<=RX46_CELL_STATUS_MAX_MS)return {...s,__rxCellFetchedAt:t};return {...s,status:'',car_status:'',condition:'',updated_at:'',__rxCellFetchedAt:t,__rxCellStale:true}}
 window.rx46CardF1b={rows:p=>rx46Rows(p),html:p=>cardHtml(p),cellAge:(s,g)=>rx46CellAge(s,g)};
 ...
 // F1B 1B.7: the card paints at once with what the drawn feature (or the search answer) already carries:
 // area, status, type, condition, fiscal modules and, from the same SICAR answer, creation and update dates.
 // A map cell read more than 5 min ago hides status, condition and update date (they change without the outline
 // changing). While the live /map-panel answer is on its way, those three keep their place as quiet
 // placeholders (value null: no label, no dash), so the card does not jump when they arrive. Once the answer
 // arrives or fails, placeholders are gone. A field a live SICAR record did not give is simply not shown.
 function rx46Rows(p){const st=statusInfo(p?.car_status||p?.status),waiting=!!p&&!!p.__rxCellStale&&!!String(p.car_code||'').trim()&&!p.__rx46Enriched&&!p.__rx46EnrichFailed,late={'Status':1,'Condição':1,'Atualização':1};return [['Área',esc(ha(p?.area_ha))],['Status',st.label?`<span class="rx46-status ${st.cls}">${esc(st.label)}</span>`:''],['Tipo',esc(typeLabel(p?.property_type||p?.type))],['Condição',esc(String(p?.condition??'').trim())],['Módulos fiscais',esc(num(p?.fiscal_modules,2))],['Criação',esc(date(p?.created_at))],['Atualização',esc(date(p?.updated_at))]].map(r=>r[1]?r:(waiting&&late[r[0]]?[r[0],null]:null)).filter(Boolean)}
 function cardHtml(p){const id=identity(p),car=id.code,C=window.rxCopyCarC2,code=C?C.button(car):esc(car),rows=rx46Rows(p);const head=id.named?`<h3 class="rx46-title">${esc(id.title)}</h3>${car?`<div class="rx46-code-row">${code}</div>`:''}`:(car?`<h3 class="rx46-title rx46-title-code">${code}</h3>`:'');return `<div class="rx46-card" data-car="${esc(car)}" data-rx-copy-scope${id.named?' data-rx-named="1"':''}${p?.__rx46Enriched?' data-rx46-enriched="1"':''}><div class="rx46-head"><span class="rx46-car-badge">CAR</span><span class="rx46-spacer"></span>${window.rxShareW1a?window.rxShareW1a.html(car,id.place):''}<button type="button" class="rx46-linkbtn" data-rx46-action="kml">Mapa KML</button><button type="button" class="rx46-x" aria-label="Fechar" data-rx46-action="close">×</button></div>${head}${id.place?`<div class="rx46-place">${esc(id.place)}</div>`:''}<button type="button" class="rx46-demo" data-rx46-action="demo" title="Abre a Consulta Pública oficial do SICAR; informe o código CAR se o site solicitar.">Consultar no SICAR (site oficial)</button>${rows.length?`<div class="rx46-grid">${rows.map(([l,v])=>v===null?`<div class="rx46-wait" data-rx46-wait="${esc(l)}" aria-hidden="true"><small>&nbsp;</small><b>&nbsp;</b></div>`:`<div class="rx46-field"><small>${l}</small><b>${v}</b></div>`).join('')}</div>`:''}${rows.length&&p?.__rxCellStale&&p?.__rx46EnrichFailed&&!p?.__rx46Enriched?`<div class="rx46-asof">${esc(rx46AsOf(p.__rxCellFetchedAt))}</div>`:''}<button type="button" class="rx46-cta" data-rx46-action="full">VER ANÁLISE COMPLETA</button>${sigefRef(p)}</div>`}
