...
(function(){
 ...
 async function enrich(localSeq,latlng){const car=String(selected?.car_code||'').trim().toUpperCase();if(!car)return;const failed=()=>{if(localSeq!==seq||!selected||!selected.__rxCellStale||selected.__rx46Enriched)return;selected={...selected,__rx46EnrichFailed:true};window.current={...selected};renderAnchor(latlng)};try{const r=await fetch(`/v1/live/map-panel/${encodeURIComponent(car)}`),d=await r.json();if(localSeq!==seq)return;if(!r.ok||!d?.ok){failed();return}selected={...selected,...d,status:d.car_status||selected.status,type:d.property_type||selected.type,geometry:d.geometry||selected.geometry,__rx46Enriched:true};window.current={...selected};replaceSelectedLayer(selected.geometry);renderAnchor(latlng);sigefFollowUp(localSeq,latlng,car)}catch(e){failed()}}
