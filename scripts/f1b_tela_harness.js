// F1B harness: runs the FINAL served scripts (full reading, panel-sources runtime, CAR integrity)
// in a node vm with a minimal fake DOM and a counting fetch. No network.
// stdin: {"f1b": "<script text>", "f2": "<script text>", "integrity": "<script text>"}
...
function analysis(car, over) {
  return Object.assign({
    ...
    prodes: { reading: { state: 'found', inside: { state: 'found', count: 2, area_ha: 3.456, years: [2006, 2021] } } },
    fire_live: { ok: true, inside_count: 0, near_count: 3, radius_km: 5, window_note: 'Últimos 6 arquivos de 10 minutos disponíveis no diretório oficial do INPE.' },
...
async function main() {
  ...
  // ---- pure rows/html
  {
    ...
    out.api = !!F && typeof F.rows === 'function' && typeof F.html === 'function' && typeof F.start === 'function';
    if (out.api) {
  ...
  // ---- single flight: five starts, three button clicks, re-renders -> ONE quick
  {
    const env = makeEnv(async (u) => u.includes('/v1/live/quick/') ? { body: { ok: true, mode: 'quick-cache', analysis: analysis(CAR), deep_state: { state: 'ready', analysis: analysis(CAR) } } } : { status: 404, body: {} });
    vm.runInContext(input.f1b, env.win);
    const F = env.win.rxFullReadingF1b;
    ...
    out.single = { quick: count(env.calls, '/v1/live/quick/'), phase: (F.peek(CAR) || {}).phase, slot_html: slot ? slot.innerHTML : null, slot_in_card: !!slot && slot.parentNode === env.card,
      pbody: env.pbody.innerHTML, button: env.button.textContent, scrolled: slot ? slot.scrolled : 0 };
  }
  // ---- deep polling: running twice, then ready
  {
    let polls = 0;
    const env = makeEnv(async (u) => {
      ...
      if (u.includes('/v1/live/progressive/status/')) { polls += 1; return { body: polls < 3 ? { state: 'running' } : { state: 'ready', analysis: analysis(CAR) } }; }
    ...
    await waitFor(settled(F));
    out.polling = { quick: count(env.calls, '/v1/live/quick/'), status: count(env.calls, '/v1/live/progressive/status/'), phase: (F.peek(CAR) || {}).phase };
  }
  // ---- failure: worker 503 -> one automatic retry, then "Consulta pendente"; no re-ask within the pause; retry asks again
  {
    ...
    F.start(CAR); env.document.dispatchEvent({ type: 'rx45:panel-rendered', detail: { car: CAR } });
    await tick(20);
