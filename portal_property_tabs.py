...
async def embargos_detail(car_code:str,request:Request=None):
    ...
    count=exact.get('occurrence_count') if answered else None
    items=[{k:x.get(k) for k in ('tad','date','type','deforestation','area_in_property_ha','geometry_origin')} for x in (exact.get('items') or [])[:20]] if answered else []
    deep=None
    ...
    return {'ok':True,'car_code':code,'ibama':{'ok':answered,'state':ib.get('source_state') or ('pending' if not answered else None),'count':count,'area_ha':exact.get('area_unique_ha') if answered else None,'items':items,'source':ib.get('source')},'autos':{'ok':autos.get('ok'),'count':autos.get('occurrence_count'),'fine_total':autos.get('fine_total'),'source':autos.get('source'),'detail':autos.get('detail'),'state':'ready' if autos else 'deep_analysis_running'},'icmbio':{'ok':ic.get('ok') is True,'count':ic.get('occurrence_count') if ic.get('ok') is True else None,'area_ha':ic.get('area_unique_ha'),'source':ic.get('source'),'detail':ic.get('detail'),'state':'ready' if ic else 'deep_analysis_running'},'note':'Consulta pendente nunca é tratada como ausência de embargo.'}
...
// T2: j() lança em QUALQUER resposta fora do 2xx — inclusive 404 e 422, em que a fonte respondeu. Por
// isso o catch de cada aba diz apenas que não deu para carregar, sem afirmar uma causa que não aconteceu.
async function j(url){const r=await fetch(url,{cache:'no-store'}),d=await r.json();if(!r.ok)throw new Error(d.detail||'sem resposta utilizável');return d}
...
const MES={JAN:'Janeiro',FEB:'Fevereiro',MAR:'Março',APR:'Abril',MAY:'Maio',JUN:'Junho',JUL:'Julho',AUG:'Agosto',SEP:'Setembro',OCT:'Outubro',NOV:'Novembro',DEC:'Dezembro'};
function dt(s){s=String(s??'');return /^\d{8}$/.test(s)?s.slice(6,8)+'/'+s.slice(4,6)+'/'+s.slice(0,4):s}
function climLine(x,period){const rec=[x.t_max_record_c!=null?`máx ${fmt(x.t_max_record_c)} °C`:'',x.t_min_record_c!=null?`mín ${fmt(x.t_min_record_c)} °C`:''].filter(Boolean).join(', ');return [x.rain_mm_day!=null?`Chuva ≈ ${fmt(x.rain_mm_month,0)} mm no mês (${fmt(x.rain_mm_day,2)} mm/dia)`:'',x.t_avg_c!=null?`temperatura média ${fmt(x.t_avg_c)} °C`:'',rec?`recorde ${period||'do período'}: ${rec}`:''].filter(Boolean).join(' · ')}
const NORMAL_MSG={janela_curta:'A comparação com o normal da época usa períodos de 30 dias ou mais.',janela_longa_sem_serie_historica:'A comparação com a média da época está disponível para períodos de até 120 dias.',fonte_recente_grade_grossa:'A chuva recente é uma estimativa regional da NASA POWER (grade de ~50 km), que pode diferir muito da chuva no imóvel; por isso não é comparada com o normal da época.'};
function normalPane(dr){if(dr.status!=='found')return `<div class="rx-note">${NORMAL_MSG[dr.reason]||'Comparação com o normal da época pendente nesta consulta.'}</div>`;const st=String(dr.state||''),series=dr.method==='percentile_series',pct=Math.round(Number(dr.percentile)),cmp=series?[pct>=50?`Mais chuva que ${pct}% dos anos`:`Menos chuva que ${100-pct}% dos anos`,`mesmos dias de ${dr.reference||''}`]:(dr.ratio_pct!=null?[`${Math.round(Number(dr.ratio_pct))}% da média`,`climatologia ${dr.reference||''}`]:null);return kpis([['Triagem',st.charAt(0).toUpperCase()+st.slice(1),series?'chuva comparada ao normal da época':'chuva comparada à média da época'],['Chuva no período',fmt(dr.rain_sum_mm)+' mm',`${dr.days} dias · ${dt(dr.period_start)} → ${dt(dr.period_end)}`],[series?'Normal da época':'Média da época',fmt(dr.normal_mm)+' mm',series?'mediana dos anos':'climatologia NASA POWER']].concat(cmp?[['Comparação',cmp[0],cmp[1]]]:[]))+`<div class="rx-note ${dr.alert?'warn':''}">${esc(dr.summary||'')}</div><div class="rx-note">${esc(dr.note||'')}</div>`}
