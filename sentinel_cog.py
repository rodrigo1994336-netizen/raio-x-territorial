...
async def _search(geometry:dict[str,Any],days:int=365):
    ...
    body={'collections':['sentinel-2-l2a'],'intersects':geometry,'datetime':f'{start.isoformat().replace("+00:00","Z")}/{end.isoformat().replace("+00:00","Z")}',
          'limit':30,'query':{'eo:cloud_cover':{'lt':45}},'sortby':[{'field':'properties.datetime','direction':'desc'}]}
    async with httpx.AsyncClient(timeout=45,follow_redirects=True,headers={'User-Agent':'Raio-X-Territorial/0.26-sentinel-cog'}) as c:
...
def _read(url:str,bounds,out_h:int,out_w:int):
    with rasterio.Env(GDAL_HTTP_MULTIRANGE='YES',GDAL_HTTP_MERGE_CONSECUTIVE_RANGES='YES',CPL_VSIL_CURL_ALLOWED_EXTENSIONS='.tif,.TIF'):
...
def _draw_boundary(img:Image.Image,gproj,out_transform):
    ...
    for g in geoms:
        ...
        if len(pts)>=3:
            ...
            draw.line(pts,fill=(18,35,24,255),width=2,joint='curve')


def _ndvi_image(ndvi:np.ndarray,inside:np.ndarray,out_path:Path,gproj,out_transform):
    h,w=ndvi.shape
    rgb=np.zeros((h,w,3),dtype='uint8');rgb[:]=[35,38,42]
    x=np.clip((ndvi+0.15)/0.95,0,1)
    # brown -> yellow -> green, only for a visual diagnostic; statistics are numeric below.
    rgb[...,0]=(180*(1-x)+45*x).astype('uint8')
    rgb[...,1]=(95*(1-x)+175*x).astype('uint8')
    rgb[...,2]=(55*(1-x)+75*x).astype('uint8')
    rgb[~inside]=[24,29,31]
    im=Image.fromarray(rgb,'RGB');_draw_boundary(im,gproj,out_transform)
    draw=ImageDraw.Draw(im,'RGBA');draw.rounded_rectangle((20,18,620,82),radius=12,fill=(4,17,12,210))
    draw.text((34,30),'NDVI REAL — SENTINEL-2',fill='white',font=_font(22,True));draw.text((34,57),'Calculado dentro do limite do CAR',fill=(210,235,218),font=_font(15))
...
async def build_sentinel_cog_property_image(car_geometry:dict[str,Any],out_path:str|Path):
    ...
    for item in items[:10]:
        ...
        try:
            with rasterio.Env(GDAL_HTTP_MULTIRANGE='YES',CPL_VSIL_CURL_ALLOWED_EXTENSIONS='.tif,.TIF'):
                with rasterio.open(red) as ref:
                    crs=ref.crs
            ...
            aspect=max((bounds[2]-bounds[0])/(bounds[3]-bounds[1]),.25)
            out_w=1100;out_h=max(650,min(1100,int(out_w/aspect)))
            r,tr,_=_read(red,bounds,out_h,out_w);g,_,_=_read(green,bounds,out_h,out_w);b,_,_=_read(blue,bounds,out_h,out_w);n,_,_=_read(nir,bounds,out_h,out_w)
            inside=geometry_mask([mapping(gproj)],out_shape=(out_h,out_w),transform=tr,invert=True)
            valid=(r>0)&(g>0)&(b>0)
            R=_scale_rgb(r,valid);G=_scale_rgb(g,valid);B=_scale_rgb(b,valid)
            rgb=np.dstack([R,G,B]);im=Image.fromarray(rgb,'RGB');_draw_boundary(im,gproj,tr)
            ...
            draw.rounded_rectangle((20,18,760,96),radius=13,fill=(4,17,12,215));draw.text((36,30),'IMAGEM ORBITAL REAL DA PROPRIEDADE',fill='white',font=_font(23,True));draw.text((36,64),f'Sentinel-2 L2A 10 m • {dt or "data não informada"} • nuvens {round(float(cloud),1) if cloud is not None else "—"}%',fill=(210,235,218),font=_font(16))
            draw.rectangle((28,out_h-62,49,out_h-41),fill=(255,238,70,255));draw.text((58,out_h-66),'Limite do CAR',fill='white',font=_font(16));draw.text((out_w-500,out_h-62),'Copernicus Sentinel-2 • AWS Open Data / Element 84',fill=(220,225,222),font=_font(13))
            im.save(out_path,'JPEG',quality=94,optimize=True)
            den=(n+r);ndvi=np.where(den>0,(n-r)/den,np.nan);vals=ndvi[inside & np.isfinite(ndvi) & (r>0) & (n>0)]
            ndvi_meta={}
            ndvi_path=out_path.with_name('ndvi_property.png')
            if vals.size:
                ndvi_meta={'ndvi_mean':round(float(np.mean(vals)),3),'ndvi_median':round(float(np.median(vals)),3),'ndvi_p10':round(float(np.percentile(vals,10)),3),'ndvi_p90':round(float(np.percentile(vals,90)),3),'ndvi_low_share_pct':round(float(np.mean(vals<.3)*100),1),'ndvi_medium_share_pct':round(float(np.mean((vals>=.3)&(vals<.55))*100),1),'ndvi_high_share_pct':round(float(np.mean(vals>=.55)*100),1),'ndvi_pixel_count':int(vals.size)}
                _ndvi_image(ndvi,inside,ndvi_path,gproj,tr)
...
print('RX_SENTINEL_COG=real_rgb_ndvi_10m',flush=True)
