...
.rx43-hero .eyebrow{display:block;margin-bottom:5px}.rx43-name{font-size:21px;line-height:1.18;margin:0;color:var(--rx43-text);overflow-wrap:anywhere}.rx43-place{margin-top:6px;color:var(--rx43-muted);font-size:10px;line-height:1.45}.rx43-code{margin-top:6px}.rx43-name-code{margin-top:6px;overflow-wrap:normal}
...
@media(max-width:720px){
 ...
 body.rx43-dossier-open #map,body.rx43-dossier-open .leaflet-control-container,body.rx43-dossier-open .hint,body.rx43-dossier-open #rxLocateBtn,body.rx43-dossier-open #rxFilterBtn,body.rx43-dossier-open #rxContextLegend,body.rx43-dossier-open #rxMapLegend,body.rx43-dossier-open #rxNetState,body.rx43-dossier-open .rx-rare-mode{display:none!important;visibility:hidden!important;pointer-events:none!important}
...
(function(){
 ...
 const area=v=>window.rxNum?window.rxNum.ha(v):'';
 ...
 function stateText(s){if(s==='confirmed')return 'Nome confirmado em fonte pública oficial.';if(s==='unresolved')return '';return 'Procurando o nome público do imóvel…'}
 function snapshotHtml(p){const id=ident(p),car=id.code,C=window.rxCopyCarC2,codeButton=C?C.button(car):esc(car);const signals=(p.main_signals||[]).filter(x=>x&&x.value!==null&&x.value!==undefined&&String(x.value).trim()!=='').slice(0,5);const sig=signals.length?signals.map(x=>`<div class="rx43-signal"><b>${esc(x.label||'Informação')}</b><span>${esc(x.value)}</span></div>`).join(''):'<div class="rx43-signal"><b>Cadastro</b><span>Dados do cadastro já na tela. A análise ambiental começa quando você pedir.</span></div>';const pending=p.public_name_state==='resolving';const kpis=[['Área',area(p.area_ha)],['Situação CAR',String(p.car_status||p.status||'').trim()],['Condição',String(p.condition||'').trim()]].filter(k=>k[1]);return `<div class="rx43-snapshot" data-car="${esc(car)}" data-rx-copy-scope><section class="rx43-hero"><span class="eyebrow">RESUMO DO IMÓVEL</span>${id.named?`<h2 class="rx43-name" id="rx43Name">${esc(id.title)}</h2>${car?`<div class="rx43-code">${codeButton}</div>`:''}`:`<h2 class="rx43-name rx43-name-code" id="rx43Name">${codeButton}</h2>`}${id.place?`<div class="rx43-place">${esc(id.place)}</div>`:''}</section>${kpis.length?`<div class="rx43-kpis">${kpis.map(([l,val])=>`<div class="rx43-kpi"><small>${l}</small><b>${esc(val)}</b></div>`).join('')}</div>`:''}<section class="rx43-section"><h4>O que sabemos agora</h4>${sig}</section>${stateText(p.public_name_state)?`<div class="rx43-state ${pending?'pending':''}" id="rx43NameState"><span class="rx43-dot"></span><span>${esc(stateText(p.public_name_state))}</span></div>`:''}<div class="rx43-state"><span class="rx43-dot"></span><span>As consultas mais demoradas só começam quando você pedir a análise completa.</span></div><div class="rx43-actions"><button type="button" class="rx43-primary" id="rx43Full">VER ANÁLISE COMPLETA</button><button type="button" class="rx43-secondary" id="rx43Pdf">RELATÓRIO PDF</button></div></div>`}
 function applySnapshot(p){const h=host();if(!h)return;const rendered=h.querySelector('.rx45-panel-card');if(rendered&&rendered.dataset.car===String(p?.car_code||'').trim().toUpperCase())return;h.innerHTML=snapshotHtml(p);q('#rx43Full')?.addEventListener('click',full);q('#rx43Pdf')?.addEventListener('click',()=>{try{window.downloadPDF?.()}catch(e){}});const title=q('#ptitle');if(title)title.textContent=ident(p).title}
 ...
 function full(){if(document.body.classList.contains('rx43-full-requested'))return;document.body.classList.add('rx43-full-requested');const body=q('#pbody');if(body&&!body.innerHTML.trim())body.innerHTML='<div class="rx43-state pending"><span class="rx43-dot"></span><span>Preparando a análise completa. O resumo acima continua na tela.</span></div>';try{if(typeof window.rxProgressiveAnalyze==='function')window.rxProgressiveAnalyze();else if(typeof analyze==='function')analyze()}catch(e){if(body)body.innerHTML='<div class="rx43-state warn"><span class="rx43-dot"></span><span>Não foi possível iniciar a análise completa agora. Tente novamente.</span></div>'}}
 function install(){
